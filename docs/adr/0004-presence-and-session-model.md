...
- One Device UUID may accumulate multiple **Device Endpoints** from different interfaces. The initiator tries distinct endpoints until TCP hello verifies the Device identity; once a Connection Request may have been sent, it must not try another endpoint and risk presenting the same request twice.
...
- Discovery is interface-aware: desktop joins and sends IPv4 multicast on each active LAN interface, targets every directed broadcast, and runs supplementary probing for each subnet not represented by a discovered Device Endpoint. A candidate found on one interface must not suppress recovery on another.
...
- Outgoing refusal, timeout, unreachable transport, busy peer, and protocol failure are distinct outcomes. Every non-cancelled failure returns to `available` with explicit user feedback.
