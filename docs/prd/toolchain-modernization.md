...
用官方推荐脚手架重建两端工程并迁回既有产品能力：桌面端采用 Electron 43 + electron-vite（Vue 3 + Pinia + 安全 IPC），移动端采用 Expo SDK 57 development build（本地 prebuild / run）。支持边界统一为 Windows 10+、macOS 14+、Android 10 / API 29+ 与 iOS 16.4+；开发工具链要求 Node.js 22.18+（22.x）或 24.3+，iOS 构建要求 Xcode 26.4+。共享协议先独立安装、检查、测试与构建，再验证两个客户端；网络行为与线协议由 `network-stack-vnext` PRD 及其 ADR 定义。
...
- 桌面：Windows 10+ x64 与 macOS 14+ x64/arm64，Electron 43；主进程承载 UDP/TCP 与系统能力；preload 只暴露窄 API；Pinia + Ant Design Vue；生产 renderer 使用受限的 `app://./` 自定义协议；旧 renderer `localStorage` 的五项产品数据在网络启动前一次性原子迁移。两端通过 `@nut-tree-fork/nut-js` 的显式映射执行 Command；macOS 要求辅助功能权限。接收文件发布使用 Koffi 提供 FFI 边界并调用各系统的原子无覆盖移动，具体持久化语义见 ADR-0006 与 ADR-0009。
...
- 桌面：`pnpm --filter syncer-desktop lint`、`typecheck`、`test`（旧数据迁移、renderer 协议边界、TCP 身份、Command 映射、原子持久化与接收文件暂存/发布适配器）、`build:win` 及 `build:mac`。
- 移动：`pnpm --filter syncer-mobile lint`、`typecheck`、`test`；本地使用 pnpm 调用 Expo Doctor、干净 prebuild 与对应平台的 run 命令。
- 持续集成在 `main` push 与 pull request 上执行协议、桌面端和移动端检查，从干净 CNG 项目运行 Android 原生模块检查与 Debug 构建，并在 Windows 与 macOS 上验证桌面端打包；pull request 不写入共享的 Gradle 或 Electron 缓存。
...
- 为 Windows 10、macOS 14、Android 10 / API 29 或 iOS 16.4 以下系统保留旧运行时、旧存储路径或兼容回退。

## Further Notes

- Android 在保持现有一键保存交互的前提下写入公共 Downloads，具体决策见 ADR-0007。
- 桌面托盘常驻与关闭即隐藏行为需保留；macOS 同时保留标准应用菜单与 Dock 激活行为。
