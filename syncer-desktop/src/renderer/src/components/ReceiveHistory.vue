<template>
  ...
  <a-modal v-model:open="visible" title="接收历史" class="receive-history" centered :footer="false">
    ...
    <div class="receive-history__list">
      ...
      <a-button v-if="hasMore" class="receive-history__show-more" type="link" @click="showMore">
        加载更多
      </a-button>
      <a-divider v-else>
...
const receiveHistory = reactive<HistoryItem[]>([])
...
const allSelected = computed(
  () => list.value.length > 0 && selectedList.value.length === list.value.length
)

/** 选择、取消选择 */
function changeSelecting(): void {
  if (!selecting.value) {
...
/** 处理记录点击事件 */
async function handleClickItem(item: HistoryItem): Promise<void> {
  if (selecting.value) {
